...
class Index extends CollectController{
    ...
    private function cj_json($code,$msg,$url,$data,$wait){
    ...
    private function cj_error($msg = '', $url = null, $data = array(), $wait = 3, array $header = []){
        if(g_sc('user')){
            $this->error($msg,$url,$data,$wait,$header);
        }else{
            
            $this->cj_json(0,$msg,$url,$data,$wait);
        }
    }
    private function cj_success($msg = '', $url = null, $data = array(), $wait = 3, array $header = []){
        if(g_sc('user')){
            $this->success($msg,$url,$data,$wait,$header);
        }else{
            
            $this->cj_json(1,$msg,$url,$data,$wait);
        }
    }
    ...
    public function auto_backstageAction(){
        ...
        if(empty($caijiConfig['auto'])){
            $this->cj_error('请先开启自动采集');
        }
        if($caijiConfig['run']!='backstage'){
            $this->cj_error('不是后台运行方式');
        }
    ...
    public function caijiAction(){
        if(is_empty(g_sc_c('caiji','auto'))){
            $this->cj_error('请先开启自动采集','admin/setting/caiji');
        }
        if(g_sc_c('caiji','run')!='visit'){
            $this->cj_error('不是访问触发方式');
        }
        ...
        if($collectVisitTime&&$nowTime-$collectVisitTime<=15){
            ...
            $this->cj_success('采集运行中',$url,null,10);
        }
        ...
        $this->cj_success('正在执行采集'.$taskIds,$url,null,10);
    }
    /*运行自动采集*/
